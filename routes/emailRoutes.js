...
import { classifyStoredEmails } from "../controllers/aiController.js";
import {
  testGroqConnection,
  getGroqModels,
...
router.post("/classify", classifyStoredEmails);
...
router.get("/groq-models", async (req, res) => {
  try {
    console.log("Testing Groq models endpoint...");
    ...
    console.error("Groq models failed:", error.message);
...
router.get("/gmail-email-details/:id", async (req, res) => {
  try {
    const email = await getEmailDetails(req.params.id);
    ...
    console.error("Email details read failed:", error.message);

    res.status(500).json({
      success: false,
      message: "Failed to read email details",
      error: error.message,
    });
  }
});
